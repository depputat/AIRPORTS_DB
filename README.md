...
Часть моделей сопоставлена с уже существующими таблицами базы данных, поэтому для них используется `managed = False` (после первого запуска проекта, рекомендуется раскомментировать данные строчки).
В файле .env некоторые строчки закомментированы, поскольку для первоначальной настройки и запуска проекта не нужны, однако их можно раскомментировать, поскольку в settings.py имеется default-значение. 
